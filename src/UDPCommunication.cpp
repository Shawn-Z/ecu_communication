...
bool UDPCommunication::init() {
    ...
    this->client_ip_len = sizeof(this->remote_addr);
...
intmax_t UDPCommunication::recv() {
    ...
    this->recv_len = recvfrom(this->sockfd, this->buffer, BUFFERSIZE, 0, (struct sockaddr *)&this->remote_addr, &this->client_ip_len);
    if (this->recv_len < 1) {
...
bool UDPCommunication::sendToClient(uint8_t *buffer, size_t send_len) {
    this->send_len = sendto(this->sockfd, buffer, send_len, 0, (struct sockaddr *)(&this->remote_addr), this->client_ip_len);
...
bool UDPCommunication::sendToRemote(uint8_t *buffer, size_t send_len) {
    this->send_len = sendto(this->sockfd, buffer, send_len, 0, (struct sockaddr *)(&), this->client_ip_len);
    if (this->send_len != send_len) {
        this->error = udp_server_error_type::send_error;
        return false;
    }
